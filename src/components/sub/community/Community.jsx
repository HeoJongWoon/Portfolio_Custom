...
// 아직 데이터베이스를 베우진 않았지만 CRUD기능을 구현하고 싶어서 로컬저장소를 활용하여 만들어 봤다.
// 이슈사항으로는 시간값을 가져왔는데 로컬저장소에 글이 저장되는 시점의 시간을 표준시로 전장을 해서 현재시간보다 9시간이 늦은 시간으로 출력되는 문제가 있었다.
// 시간값을 변경하려고 보니 JSON.parse로 객체형태로 시간이 불러와져서 split 메서드를 쓸수가 없는데 이유를 몰라서 시간을 많이 낭비했다.
// 객체형태로 변환된 값을 다시 stringify로 문자화시킨다음에 split으로 문자값 가공하고 다시 화면에 출력
// 두번째 이슈사항으로 다른 컴퓨터로 내 작업물을 확인해보니 해당 브라우저에는 저장된 데이터가 없어서 커뮤니티 페이지가 빈화면으로 출력되는 문제가 있었다.
// 로컬저장소에 값이 없을 때 더미 데이터가 출력되도록 했다.
