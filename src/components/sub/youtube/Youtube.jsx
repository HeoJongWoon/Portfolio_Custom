// 해당 유튜브 페이지 작업에 대해서 설명, 이슈사상은 없었는지
...
// 내가 느끼기에는 너무 동작방식이 중앙집중적이고 간단한 비동기 데이터를 전역관리하기에는 코드에 복잡도가 커서 비효율적으로 느껴짐
// 대안으로 redux-toolkit이라는 것을 활용했다. 비동기데이터의 상태에 따라서 자동으로 액션객체를 생성해주고 액견객체의 상태에 따라서 리듀서 알아서 전역데이터를 변경해주는 방식이 효육적으로 느껴져서 적용을 해봤다.
// redux-toolkit을 활용함으로써 컴포넌트안쪽에서 비동기 데이터 함수를 관리하는게 아닌 컴포넌트 외부의 slice파일을 통해서 컴포넌트 외부에서 비동기데이터별로 fetching함수의 리듀서 함수를 한번에 관리할 수 있는 부분이 편하게 느껴졌다.
